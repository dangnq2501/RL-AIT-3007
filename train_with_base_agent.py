...
from models.functional_model import FunctionalPolicyAgent, BasePolicyAgent
...
if __name__ == "__main__":
    ...
    base_agent = BasePolicyAgent(action_space_size, input_dim)
    base_agent.q_network.load_state_dict(
        torch.load("trained_agent.pth", weights_only=True, map_location="cpu")
    )
    base_agent.eval()
    
    training_agent = FunctionalPolicyAgent(action_space_size, input_dim)
    ...
    for episode in tqdm(range(n_episodes), desc="Training episodes"):
        ...
        for agent_name in env.agent_iter():
            ...
            if termination or truncation:
                ...
                if agent_team == "blue":
                    ...
                    action = training_agent.select_action(observation_tensor)
                    ...
                    observation_tensor = torch.tensor(observation, dtype=torch.float32).flatten()
                    action = base_agent.select_action(observation_tensor)
        ...
        if len(replay_buffer) >= batch_size:
            ...
            trainer.fit(training_agent, dataloader)
    ...
    #             observation_tensor = torch.tensor(observation, dtype=torch.float32).flatten()
    #             action = training_agent.select_action(observation_tensor, eval_mode=True)
    #         else:
    #             # Red agents use a random policy
    ...
    torch.save(training_agent.q_network.state_dict(), "trained_agent.pth")
